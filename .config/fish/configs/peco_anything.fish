...
function __peco_complete_cmd_anything
...
bind \cg __peco_complete_cmd_anything

function __create_dracula_theme_prompt
...
function __call_cmd_peco_tmux_popup
...
function __peco_git_unstaged_file_dir
    ...
    set -l popup_status (__create_dracula_theme_prompt $sub_prompt)
    set -l list_cmds "git status -uall --porcelain"
    set -l select_cmd "awk '{print \$NF}'"

    set -l target (__call_cmd_peco_tmux_popup $list_cmds $select_cmd $popup_status)
